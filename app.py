...
#     command = f'edge-tts --voice "{voice}" --text "{text}" --write-media "Virtual_Assistant/output.mp3"'
...
#     try:
#         pygame.mixer.music.load("Virtual_Assistant/output.mp3")
...
# cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # 0 refers to the default webcam
...
#         self.timer = QTimer()
#         self.timer.timeout.connect(self.update_webcam_feed)
#         self.timer.start(.5)  # Update the webcam feed every 50 milliseconds
